...
func main() {
	...
	for scanner.Scan() {
		...
		if coordinates[0] == coordinates[2] {
			...
			steps := int(math.Abs(float64(coordinates[2] - coordinates[0])))
			factor_x, factor_y := calculate_factors(coordinates)
			for offset := 0; offset <= steps; offset++ {
				x := coordinates[0] + (offset * factor_x)
				y := coordinates[1] + (offset * factor_y)
				ocean_floor = ensure_size(ocean_floor, x, y)
				ocean_floor.vent_count[x][y]++
			}
		}
	}
...
func calculate_factors(coordinates [4]int) (int, int) {
	return calculate_factor(coordinates[0], coordinates[2]), calculate_factor(coordinates[1], coordinates[3])
}
func calculate_factor(a int, b int) int {
	if a < b {
		return 1
	}
	return -1
